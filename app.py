    ...
    dag=dag,
            ...
            if dependency_flow == "Parallel":
                ...
                for step in ordered_steps:
                    if len(step) == 1:
                        dependency_parts.append(task_name_map[step[0]])
                    else:
                        task_vars = ", ".join([task_name_map[tid] for tid in step])
                        dependency_parts.append(f"[{task_vars}]")
                dependency_chain = " >> ".join(dependency_parts)
